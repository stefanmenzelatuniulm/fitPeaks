...
rf_center_ppm = 170.54; %Center of the RF pulse (ppm) %IST SCHON RELATIV ZU TMS
bw = 1000; %Bandwidth of the RF pulse (Hz)
...
floorOffset = 0.015;
path = "D:\Matlab\MR_Data\Preliminary Measurements\Overview\WithEthylacetate\198"; %Path to data
attemptLorentzianFit = true; %Attempt to fit single Lorentzian
attemptFIDFit = false;
notOutlierDomain = [165 170];
ppmyoffset = -0.03;
minPeakHeight = 0.489;
additionalFileString="EthylacetateFit";
useFindPeaks = false;
...
coeffs=["FWHM" "x0" "COff"];
options=fitoptions('Method','NonlinearLeastSquares','Lower',[-inf -inf ppmyoffset],'Upper',[inf inf ppmyoffset],'StartPoint',[1 rf_center_ppm ppmyoffset]);
%coeffs=["FWHM" "x0"];
...
if attemptLorentzianFit
    ...
    plot(ax,ft,"r");
    if useFindPeaks
        [~,~,FWHMofPeak,~]=findpeaks(sReal(~outliersLorentzian),X_ppm_t(~outliersLorentzian),'Annotate','extents','WidthReference','halfheight','MinPeakHeight',minPeakHeight);
        findpeaks(sReal(~outliersLorentzian),X_ppm_t(~outliersLorentzian),'Annotate','extents','WidthReference','halfheight','MinPeakHeight',minPeakHeight);
        ax = gca;
